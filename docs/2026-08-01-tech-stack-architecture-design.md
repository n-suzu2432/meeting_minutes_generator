...
## 短尺音声によるE2Eスモークテスト成功(5回目)

### 背景

OpenAIアカウントに残高がなく`insufficient_quota`エラーで文字起こしが失敗していた件、ユーザーが残高を追加。これを受けて、Windows標準のTTS(`Microsoft Haruka Desktop`、日本語音声)で5フレーズの模擬会議音声(約30秒、決定事項1件・TODO1件を含む内容)を合成し、パイプライン全体を検証した。

### 実装内容

- PowerShellの`System.Speech.Synthesis.SpeechSynthesizer`で模擬会議音声(`test_meeting.wav`)を生成
- まず`curl`で`POST /api/minutes`に直接投げてバックエンド単体を検証 → 成功
- 続けてブラウザ(Chrome自動操作)で`http://localhost:3000`から実際にファイル選択・アップロード・「議事録を作成する」ボタン押下 → ローディング表示 → 結果表示 → Markdownダウンロードまでの一連の操作をUI経由で確認
- テスト後、生成した音声ファイル・ダウンロードした`minutes.md`・APIレスポンスの一時ファイルはすべて削除してクリーンアップ

### 結果

- Whisperの文字起こしはタイムスタンプ付き(`[00:00:00]`〜`[00:00:29]`)で発話内容を正確に書き起こした
- Claude(`claude-sonnet-5`)は「決定事項」「TODO(担当者・期限含む)」を自然文から正しく構造化抽出できた(例: TODO「資料の作成」の担当者「鈴木さん」・期限「来週の金曜日」を音声内の自然な言い回しから正しく抽出)
- `failed_chunk_count: 0`、Whisper概算コスト約$0.003、Claude概算コスト約$0.0056
- ブラウザUIでも同じ結果がMarkdownレンダリング(`react-markdown` + Tailwind Typography)込みで正しく表示され、ダウンロードボタンも`minutes.md`を正しく生成した

### 残課題

- 1時間規模の実音声でのE2E検証(「5分以内」の達成基準)は未実施。今回は約30秒の短尺音声のみでの検証
- チャンク分割(無音検出)・並列処理は、今回1チャンクで完結する短尺音声だったため実質未検証。複数チャンクに分割される長さの音声での検証が今後必要
